...
I2CManager::I2CManager(i2c_port_t i2c_num, gpio_num_t sda_pin, gpio_num_t scl_pin, uint32_t freq_hz)
...
esp_err_t I2CManager::init() {
    ...
    esp_err_t ret = i2c_param_config(i2c_num_, &i2c_conf);
    if (ret != ESP_OK) return ret;

    return i2c_driver_install(i2c_num_, i2c_conf.mode, 0, 0, 0);
}

esp_err_t I2CManager::write(uint8_t device_addr, uint8_t* data, size_t length) {
...
esp_err_t I2CManager::read(uint8_t device_addr, uint8_t* data, size_t length) {
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    esp_err_t ret = ESP_OK;
